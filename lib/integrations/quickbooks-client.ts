...
/**
 * Retrieves QuickBooks OAuth tokens from Supabase
 */
...
export async function refreshQuickBooksAccessToken(
  ...
  const existingTokens = await getQuickBooksTokens(tenantId)
  ...
  // Return fresh tokens
  const newTokens = await getQuickBooksTokens(tenantId)
  if (!newTokens) {
...
/**
 * Gets valid QuickBooks tokens, refreshing if necessary
 */
export async function getValidQuickBooksTokens(
  tenantId: string
): Promise<QuickBooksTokens> {
  let tokens = await getQuickBooksTokens(tenantId)
...
/**
 * Creates a QuickBooks API client with automatic token refresh
 */
export async function createQuickBooksClient(tenantId: string) {
  const tokens = await getValidQuickBooksTokens(tenantId)
