...
function createLossesSheet(lossSummary: any): LossesSheet {
  ...
  const data = lossSummary.lossHistory.map((loss: any) => [
    ...
    loss.cotSbtAnalysis.cotSatisfied === 'unknown' ? 'Unknown' : loss.cotSbtAnalysis.cotSatisfied ? 'Yes' : 'No',
    loss.cotSbtAnalysis.sbtRequired === 'unknown' ? 'Unknown' : loss.cotSbtAnalysis.sbtRequired ? 'Yes' : 'No',
    loss.cotSbtAnalysis.sbtSatisfied === 'unknown' ? 'Unknown' : loss.cotSbtAnalysis.sbtSatisfied ? 'Yes' : 'No',
...
function createDiv7aSheet(div7aSummary: any): Div7aSheet {
  ...
  const data = div7aSummary.loanAnalyses.map((loan: any) => [
    ...
    loan.hasWrittenAgreement === 'unknown' ? 'Unknown - Verify' : loan.hasWrittenAgreement ? 'Yes' : 'No',
