...
| A-6 | cgt-engine | ~~Connected entity aggregation for $6M net asset test (Subdivision 152-15) not implemented~~ **FIXED** (2026-02-08): `CGTAnalysisOptions.connectedEntities` accepts array of connected entities/affiliates. `analyzeDivision152()` aggregates net assets per Subdivision 152-15. `Div152Analysis.netAssetTest` expanded with `connectedEntityAssets`, `aggregatedNetAssets`, `cliffEdgeWarning`, and entity `breakdown`. Warns when within 10% of $6M cliff edge. | HIGH | Fixed |
| A-7 | cgt-engine | ~~Collectable/personal use asset loss quarantining (s 108-10, s 108-20) not distinguished~~ **FIXED** (2026-02-08): `classifyAssetCategory()` classifies events as collectable/personal_use/other. Collectable losses quarantined to only offset collectable gains (s 108-10(1)). Personal use losses completely disregarded (s 108-20(1)). `CGTEvent` gains `assetCategory` + `assetCategoryNote`. `CGTSummary` gains 5 quarantined loss fields. | MEDIUM | Fixed |
| A-8 | cgt-engine | Cost base always initialised at 0 -- no asset register integration | MEDIUM | By design (requires data) |
| A-9 | loss-engine | ~~Trust losses (Division 266/267) not distinguished from company losses (Division 165)~~ **FIXED** (2026-02-08): `analyzeCotSbt()` now entity-type-aware. Trust entities routed to `analyzeTrustLossRecoupment()` using Division 266/267 Schedule 2F ITAA 1936 (not Division 165). `CotSbtAnalysis` gains `trustLossRule`, `familyTrustElection`, `trustNotes` fields. Trust-specific recommendations reference family trust election (s 272-75), pattern of distributions test (s 269-60), and income injection test (Division 270). | MEDIUM | Fixed |
...
| A-12 | div7a-engine | ~~Distributable surplus cap (s 109Y) -- deemed dividend cannot exceed distributable surplus~~ **FIXED** (2026-02-08): `estimateDistributableSurplus()` queries equity account data; `calculateDiv7aSummary()` caps `totalDeemedDividendRisk` at distributable surplus per s 109Y. Optional `knownDistributableSurplus` parameter for callers with balance sheet data. Fields added: `distributableSurplus`, `distributableSurplusSource`, `distributableSurplusNote`, `cappedTotalDeemedDividendRisk`, `cappedTotalPotentialTaxLiability`. Warnings when surplus unknown. | HIGH | Fixed |
...
| C-2 | Shared reports | ~~Dollar estimates with no disclaimer~~ **FIXED** (2026-02-08): Added `estimateDisclaimer` field to ExecutiveSummary, MetricCard labelled "(Est.)", disclaimer rendered in UI | MEDIUM |
...
| ~~P2-2~~ | ~~Implement connected entity aggregation for CGT $6M test~~ **DONE** (2026-02-08) -- `connectedEntities` option + aggregation + cliff edge warning | ~~5 days~~ |
