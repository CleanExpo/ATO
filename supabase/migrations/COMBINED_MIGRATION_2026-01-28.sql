-- ================================================================
-- COMBINED MIGRATION - Multi-Platform Support + Tax Alerts
-- ================================================================
-- Created: 2026-01-28
-- Purpose: Apply all pending migrations in one transaction
--
-- Includes:
-- 1. Platform column support (Xero, MYOB, QuickBooks)
-- 2. Platform columns for analysis tables
-- 3. QuickBooks OAuth tokens table
-- 4. Tax Alerts System (automated notifications)
--
-- How to apply:
-- 1. Go to: https://app.supabase.com/project/xwqymjisxmtcmaebcehw/sql/new
...
-- ================================================================
-- MIGRATION 1: Add Platform Column Support
-- ================================================================
...
-- ================================================================
-- MIGRATION 2: Add Platform to Analysis Tables
-- ================================================================
...
-- Use created_at instead of analysis_date to ensure compatibility
...
-- ================================================================
-- MIGRATION 3: QuickBooks OAuth Tokens Table
-- ================================================================
...
-- RLS Policy: Users can only access their own tokens (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'quickbooks_tokens' AND policyname = 'Users can access own QuickBooks tokens') THEN
    CREATE POLICY "Users can access own QuickBooks tokens" ON quickbooks_tokens FOR ALL USING (auth.uid() = tenant_id);
  END IF;
END $$;
...
-- Verification Query (run this separately to check success)
-- SELECT table_name FROM information_schema.tables
-- WHERE table_schema = 'public'
-- AND table_name IN ('quickbooks_tokens', 'tax_alerts', 'tax_alert_definitions', 'tax_alert_preferences')
-- ORDER BY table_name;
